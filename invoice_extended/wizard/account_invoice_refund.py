            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                if inv.reconciled and mode in ('cancel', 'modify'):
                    raise UserError(_('Cannot create a credit note for the'
                                      ' invoice which is already reconciled,'
                                      ' invoice should be unreconciled first,'
                                      ' then only you can add credit note '
                                      'for this invoice.'))
                ...
                if mode in ('cancel', 'modify'):
                    ...
                    if mode == 'modify':
                        ...
                        body = _('Correction of <a href=# data-oe-model='
                                 'account.invoice data-oe-id=%d>%s</a><br>'
                                 'Reason: %s') % (
                ...
                xml_id = \
                    ...
                    inv.type == 'out_account_collection' and \
                    'action_invoice_out_refund_account_collection' or \
                    inv.type == 'out_refund_account_collection' and \
                    'action_invoice_out_account_collection'

        if xml_id:
            if inv.type in ['out_account_collection',
                            'out_refund_account_collection']:
                result = self.env.ref(
                    'invoice_extended.%s' % (xml_id)).read()[0]
            else:
                result = self.env.ref('account.%s' % (xml_id)).read()[0]
            if mode == 'modify':
                # When refund method is `modify` then it will directly
                # open the new draft bill/invoice in form view
                if inv_refund.type == 'in_invoice':
                    view_ref = self.env.ref('account.invoice_supplier_form')
                elif inv_refund == 'out_account_collection':
                    view_ref = self.env.ref(
                        'invoice_extended.invoice_form_out_account_collection')
                else:
