...
class db_backup(models.Model):
    ...
    def get_db_list(self, host, port, context={}):
        # uri = 'http://' + host + ':' + port
        uri = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
    ...
    @api.multi
    def generate_backup_for_S3(self, DB, LOCAL_FOLDER, BACKUP_TYPE, S3_Folder):
        ...
        try:
            ...
            raise
        bucket_name = \
            self.env['ir.attachment']._get_s3_settings('s3.bucket',
                                                       'S3_BUCKET')
        s3 = self.env['ir.attachment']._get_s3_resource()

        if not s3:
            _logger.info('something wrong on aws side, Please check the s3 configurations')
            return True

        # Create name for dumpfile.
        bkp_file = '%s_%s.%s' % (time.strftime('%Y_%m_%d_%H_%M_%S'), DB, BACKUP_TYPE)
        file_path = os.path.join(LOCAL_FOLDER, bkp_file)
        url = ''
        try:
            # try to backup database and write it away
            fp = open(file_path, 'wb')
            odoo.service.db.dump_db(DB, fp, BACKUP_TYPE)
            fp.close()
        except Exception as error:
            _logger.debug(
                "Couldn't backup database %s. Bad database administrator password for server running at %s" % (
                DB, url))
            ...
            if DB in f:
                ...
                try:
                    s3.meta.client.upload_file(
                        fullpath,
                        bucket_name,
                        Key='{1}/{0}'.format(f, S3_Folder))
                except Exception as e:
                    raise exceptions.UserError(e)
                url = \
                    self.env['ir.attachment']._get_s3_object_url(
                        s3, bucket_name, f, S3_Folder)
                object = s3.Object(bucket_name, '{1}/{0}'.format(f, S3_Folder))
                ...
                # size = round(file_size / power, 2)
                # _logger.info(
                #     "Delete local file: " + fullpath)
                os.remove(fullpath)
        return True
    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            if rec.name in db_list:
                ...
                _logger.debug("database %s doesn't exist on %s" % (rec.name, url))

            if rec.s3_write:
                bucket_name = \
                    self.env['ir.attachment']._get_s3_settings('s3.bucket',
                                                               'S3_BUCKET')
                folder_name = rec.s3_folder
                s3 = self.env['ir.attachment']._get_s3_resource()
                dir = rec.folder
                for f in os.listdir(dir):
                    if rec.name in f:
                        ...
                        try:
                            s3.meta.client.upload_file(
                                fullpath,
                                bucket_name,
                                Key='{1}/{0}'.format(f, folder_name))
                        except Exception as e:
                            raise exceptions.UserError(e.message)
                        url = \
                            self.env['ir.attachment']._get_s3_object_url(
                                s3, bucket_name, f, folder_name)
                        object = s3.Object(bucket_name, '{1}/{0}'.format(f, folder_name))
                        # file_size = object.content_length
                        # size_name = (
                        #     "B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB",
                        #     "YB")
                        ...
                        # power = math.pow(1024, i)
                        # size = round(file_size / power, 2)
                        # self.env['db.backup.history'].create({'name': f,
                        #                                       'size': "%s %s" % (size, size_name[i]),
                        #                                       'url': url})
                        _logger.info(
                            "Delete local file: " + fullpath)
                        os.remove(fullpath)
