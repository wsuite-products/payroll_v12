    ...
    def _inverse_datas(self):
        if not self.env.context.get('force_s3'):
            s3_records = self.sudo().search([('id', 'in', self.ids),
                                             ('res_id', '>', 0),
                                             ('res_model', 'in', ['order.line.comment', 'workorder.comment'])])
        else:
            s3_records = self
        ...
        OrderLineComment = self.env['order.line.comment'].sudo()
        WorkOrderComment = self.env['workorder.comment']
        for attach in self & s3_records:
            ...
            url_data = self._get_s3_object_url(s3, bucket_name, fname,
                                               folder_name)
            if self.res_model == 'order.line.comment':
                _logger.info("attach Order Comment ==> %s", attach)
                comment_id = OrderLineComment.search([('id', '=', self.res_id)])
                comment_id.attachment_url = url_data
            if self.res_model == 'workorder.comment':
                _logger.info("attach WorkOrder Comment ==> %s", attach)
                comment_id = WorkOrderComment.search([('id', '=', self.res_id)])
                comment_id.attachment_url = url_data
            vals = {
