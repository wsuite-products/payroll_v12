...
class BaseSynchro(models.TransientModel):
    ...
    def synchronize(self, server, object):
        ...
        for dt, id, action in sync_ids:
            ...
            if id2:
                ...
                if not destination_inverted:
                    if object.context:
                        idnew = pool_dest.env[object.model_id.model].with_context(eval(object.context)).create(value)
                    else:
                        idnew = pool_dest.env[object.model_id.model].create(value)
                    if object.default_value and idnew:
                        idnew.write(eval(object.default_value))
                    if idnew:
                        new_id = idnew.id
                        if object.method and idnew:
                            for method_name in object.method.split(','):
                                method_str = """pool_dest.env['""" + str(object.model_id.model) + """'].browse(""" + str(new_id) + """).""" + str(method_name)
                                eval(method_str)
                else:
                    if object.context:
                        idnew = pool_dest.get(object.model_id.model).with_context(eval(object.context)).create(value)
                    else:
    ...
    def upload_download(self):
        ...
        server = \
            self.env['base.synchro.server'].browse(syn_obj.server_url_id.id)
...
Exceptions:
        ''' % (start_date, end_date, self.report_total, self.report_write,
            ...
            request.create({
                ...
                'act_from_id': self.user_id.id,
                'date': time.strftime('%Y-%m-%d, %H:%M:%S'),
                'act_to_id': syn_obj.user_id.id,
                'body': summary,
            })
            return {}
