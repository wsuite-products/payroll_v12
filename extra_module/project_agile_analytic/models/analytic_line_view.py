...
class ProjectAnalyticLineView(models.Model):
    ...
    task_id = fields.Many2one(
        comodel_name="project.task", string="Task", readonly=True,
    )

    type_id = fields.Many2one(
        comodel_name="project.task.type2", string="Type", readonly=True,
    )
    ...
    stage_id = fields.Many2one(
        comodel_name="project.task.type", string="Stage", readonly=True,
    )

    user_id = fields.Many2one(
        comodel_name="res.users", string="Assignee", readonly=True,
    )
    ...
    duration = fields.Float(string="Duration in hours", readonly=True,)
