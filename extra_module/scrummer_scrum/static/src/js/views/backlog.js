...
odoo.define('scrummer_scrum.view.backlog', function (require) {
    ...
    const DataWidget = require('scrummer.BaseWidgets').DataWidget;
    ...
    const ScrumBacklogView = BacklogView.AbstractBacklogView.extend({
        ...
        loadDependencies() {
            return this._super().then(() => {
                ...
                // return data.xmlidToResId("scrummer.project_task_type_epic").then(epic_type_id => this.epic_type_id = epic_type_id);
            });
        ...
        sprintBacklogStatesFilterExists() {
            return this.board.scrum_backlog_state_ids && this.board.scrum_backlog_state_ids.length > 0;
        },
        ...
        getBacklogTaskDomain() {
            return this._super().then(domain => {
                ...
                if (this.sprintBacklogStatesFilterExists()) {
                    domain.push(["wkf_state_id", "in", this.board.scrum_backlog_state_ids]);
                }
        ...
        getApplyFilterDomain() {
            return this._super().then(domain => {
                ...
                if (this.sprintBacklogStatesFilterExists()) {
                    ...
                    domain.splice(domain.length - 1, 0, "&");
                    domain.splice(domain.length, 0, states_domain);
                }
