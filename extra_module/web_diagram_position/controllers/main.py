...
class MyDiagramView(DiagramView):
    @odoo.http.route()
    def get_diagram_info(
        self, id, model, node, connector, src_node, des_node, label, **kw
    ):
        ...
        data_connectors = connector_model.search([("id", "in", list_tr)]).read(
            connector_fields
        )
