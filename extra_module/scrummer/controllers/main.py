...
class ScrummerController(http.Controller):
    @http.route("/scrummer/web", type="http", auth="user")
    def index(self, debug=False, **k):
        context = {
            "session_info": json.dumps(request.env["ir.http"].session_info())
        }

        return request.render("scrummer.index", qcontext=context)

    @http.route(
        ["/scrummer/web/data/task/create_link"], type="json", auth="user"
    )
    ...
    @http.route(
        ['/scrummer/web/data/task/<model("project.task"):task>/add_comment'],
        type="json",
        auth="user",
    )
    def add_comment(self, task, comment):
        ...
        return new_message.with_context(scrummer=True).message_format()[0]

    @http.route(
        [
            '/scrummer/web/data/task/<model("project.task"):task>/update_comment/'
    ...
    def edit_comment(self, task, message, comment):
        ...
        if "attachment_ids" in comment:
            if isinstance(comment["attachment_ids"], list):
                attachment_ids = (
                    message.attachment_ids.ids + comment["attachment_ids"]
                )
                comment["attachment_ids"] = [(6, 0, attachment_ids)]
            else:
                del comment["attachment_ids"]
        message.write(comment)
        return message.with_context(scrummer=True).message_format()[0]

    @http.route(
        ['/scrummer/web/data/workflow/<model("project.workflow"):workflow>'],
        type="json",
        auth="user",
    )
    ...
    def confirm_task_stage_change(
        self, task, values, message=None, log_message=False
    ):
        ...
        if message:
            msg = task.message_post(
                body=message, message_type="comment", subtype=subtype
            )

        return (
            msg
            and msg.with_context(scrummer=True).message_format()[0]
            or False
        )

    @http.route(
        [
            '/scrummer/web/data/task/<model("project.task"):task>/get_task_links',
        ],
        type="json",
        auth="user",
    )
    ...
    @http.route(
        [
            '/scrummer/web/data/project/<model("project.project"):project>'
    ...
    def task_types_and_priorities(self, project):
        ...
        for type in project.type_id.task_type_ids:
            ...
            for priority in type.priority_ids:
                result["priorities"][priority.id] = self.prepare_task_priority(
                    priority
                )
    ...
    def prepare_task_type(self, type):
        return {
            ...
            "scrummer_icon": type.scrummer_icon,
            "scrummer_icon_color": type.scrummer_icon_color,
    ...
    def prepare_task_priority(self, priority):
        return {
            "id": priority.id,
            "name": priority.name,
            "scrummer_icon": priority.scrummer_icon,
            "scrummer_icon_color": priority.scrummer_icon_color,
        }
    ...
    def prepare_task(self, task):
        return {
            ...
            "type_scrummer_icon": task.type_scrummer_icon,
            "type_scrummer_icon_color": task.type_scrummer_icon_color,
            "priority_scrummer_icon": task.priority_scrummer_icon,
            "priority_scrummer_icon_color": task.priority_scrummer_icon_color,
    ...
    def load_messages(
        self, model, res_id, message_type=None, message_subtype=None, **kwargs
    ):
        def operator(value):
        ...
        return (
            ...
            .with_context(scrummer=True)
            .message_format()
        )

    @http.route("/scrummer/activity-stream", type="json", auth="user")
            ...
            .search(
                [
                    (
                        "project_id",
                        "in",
                        request.env.user.team_id.project_ids.ids,
                    )
                ]
            )
            .ids
    ...
    @http.route("/scrummer/model/info", type="json", auth="user")
    def scrummer_model_info(self, model_name):
        ...
        for field_name in model_obj._fields:
            ...
            if not field._attrs.get("scrummer", False):
                continue
    ...
    def get_task_url(self, key):
        ...
        task_url = "scrummer/web?#page=board&project=%s&task=%s&view=task" % (
    ...
    def get_project_url(self, key):
        ...
        project_url = "scrummer/web?#page=board&project=%s&view=%s" % (
    ...
    @http.route("/scrummer/browse/<string:key>", type="http", auth="user")
