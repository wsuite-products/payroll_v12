    ...
    def _inverse_datas(self):
        if not self.env.context.get('force_s3'):
            s3_records = self.sudo().search([('id', 'in', self.ids),
                                             ('res_id', '>', 0),
                                             ('res_model', 'in', ['step.page.comment', 'crm.stage'])])
        else:
            s3_records = self
        ...
        StepPageComment = self.env['step.page.comment'].sudo()
        ...
        for attach in self & s3_records:
            ...
            url_data = self._get_s3_object_url(s3, bucket_name, fname,
                                               folder_name)
            if self.res_model == 'step.page.comment':
                _logger.info("attach Comment ==> %s", attach)
                comment_id = StepPageComment.search([('id', '=', self.res_id)])
                comment_id.attachment_url = url_data
            if self.res_model == 'crm.stage':
                _logger.info("attach Stage==> %s", attach)
                crm_stage_id = CRMStage.search([('id', '=', self.res_id)])
                crm_stage_id.image_url = url_data
            vals = {
