...
class ProductPricelistItem(models.Model):
    ...
    negotiation_percentage = fields.Float('Negotiation Percentage')
    tag_id = fields.Many2one('crm.product.categ.tag')
    audience_id = fields.Many2one('crm.audience', 'Audience')
...
class ProductPricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                if rule.base == 'vendor_price':
                    selected_seller = self.env['product.supplierinfo'].search([('supplier_selected', '=', True)], limit=1)
                    if not product.seller_ids:
                        continue
                    elif selected_seller.name.id != rule.supplier_id.id:
                        continue
