odoo.define('step.result', function (require) {
'use strict';

require('web.dom_ready');

if(!$('.js_stepresult').length) {
    return $.Deferred().reject("DOM doesn't contain '.js_stepresult'");
}

    console.debug("[step] Step Result JS is loading...");

    //Script For Pagination
    var step_pagination = $('.pagination');
    $.each(step_pagination, function(index, pagination){
    ...
    //load chart to svg element chart:initialized chart, response:AJAX response, quistion_id:if of step question, tick_limit:text length limit
    ...
    //Script For Graph
    var step_graphs = $('.step_graph');
    $.each(step_graphs, function(index, graph){
    ...
    $('.clear_step_filter').click(function(){
    ...
    // toggle finished/all steps filter
    ...
    console.debug("[step] Step Result JS loaded!");
