...
class IrAttachment(models.Model):
    ...
    def _inverse_datas(self):
        ...
        MultiBrand = self.env['multi.brand'].sudo()
        for attach in self & s3_records:
            _logger.info("attach multi ==> %s", attach)
            ...
            url_data = self._get_s3_object_url(s3, bucket_name, fname,
                                               folder_name)
            if self.res_model == 'multi.brand':
                brand_id = MultiBrand.search([('id', '=', self.res_id)])
                brand_id.image_url = url_data
            vals = {
